  ...
  >
    ...
    <nav class="flex-1 py-4 space-y-1 px-2">
      <router-link
        ...
        @mouseenter="!isActive(item.path) && ($event.target as HTMLElement).style.setProperty('background-color', 'var(--color-bg-hover)')"
        @mouseleave="!isActive(item.path) && ($event.target as HTMLElement).style.setProperty('background-color', 'transparent')"
      >
        ...
        <svg v-else-if="item.icon === 'certificate'" class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
        </svg>
        <svg v-else-if="item.icon === 'ca'" class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
        </svg>
        <svg v-else-if="item.icon === 'dns'" class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <svg v-else-if="item.icon === 'monitor'" class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="p-3" :style="{ borderTop: '1px solid var(--color-border)' }">
      <div v-if="!collapsed" class="flex items-center justify-between mb-2 px-1">
        ...
        >
          ...
          <span>{{ currentTheme === 'dark' ? t('theme.dark') : currentTheme === 'light' ? t('theme.light') : t('theme.auto') }}</span>
        </button>
        ...
        <div class="relative">
          ...
          <div
            v-if="showLangMenu"
            class="absolute bottom-full right-0 mb-1 rounded-lg shadow-lg py-1 min-w-[100px] z-50"
            :style="{ backgroundColor: 'var(--color-bg-surface)', border: '1px solid var(--color-border)' }"
          >
            <button
              @click="setLocale('zh-CN'); showLangMenu = false"
              class="w-full text-left px-3 py-1.5 text-xs transition-colors"
              :style="{ color: currentLocale === 'zh-CN' ? 'var(--color-primary-400)' : 'var(--color-text-primary)' }"
              @mouseenter="($event.target as HTMLElement).style.setProperty('background-color', 'var(--color-bg-hover)')"
              @mouseleave="($event.target as HTMLElement).style.setProperty('background-color', 'transparent')"
            >
              {{ t('lang.zh') }}
            ...
            <button
              @click="setLocale('en-US'); showLangMenu = false"
              class="w-full text-left px-3 py-1.5 text-xs transition-colors"
              :style="{ color: currentLocale === 'en-US' ? 'var(--color-primary-400)' : 'var(--color-text-primary)' }"
              @mouseenter="($event.target as HTMLElement).style.setProperty('background-color', 'var(--color-bg-hover)')"
              @mouseleave="($event.target as HTMLElement).style.setProperty('background-color', 'transparent')"
            >
              {{ t('lang.en') }}
            ...
            <button
              @click="setLocale('auto'); showLangMenu = false"
              class="w-full text-left px-3 py-1.5 text-xs transition-colors"
              :style="{ color: currentLocale === 'auto' ? 'var(--color-primary-400)' : 'var(--color-text-primary)' }"
              @mouseenter="($event.target as HTMLElement).style.setProperty('background-color', 'var(--color-bg-hover)')"
              @mouseleave="($event.target as HTMLElement).style.setProperty('background-color', 'transparent')"
            >
              {{ t('lang.auto') }}
      ...
      <div v-else class="flex flex-col items-center gap-1">
        <button
          ...
          :style="{ color: 'var(--color-text-secondary)' }"
          :title="currentTheme === 'dark' ? t('theme.light') : currentTheme === 'light' ? t('theme.auto') : t('theme.dark')"
        >
          <svg v-if="currentTheme === 'dark'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
          </svg>
        ...
        <div class="relative">
          ...
          <div
            v-if="showLangMenu"
            class="absolute left-full top-0 ml-2 rounded-lg shadow-lg py-1 min-w-[100px] z-50"
            :style="{ backgroundColor: 'var(--color-bg-surface)', border: '1px solid var(--color-border)' }"
          >
            <button
              @click="setLocale('zh-CN'); showLangMenu = false"
