// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
// This file is automatically generated. DO NOT EDIT

// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore: Unused imports
import * as deploy$0 from "./internal/deploy/models.js";
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore: Unused imports
import * as deploycredential$0 from "./internal/deploycredential/models.js";
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore: Unused imports
import * as dnsprovider$0 from "./internal/dnsprovider/models.js";

/**
 * ApplyCertRequest 申请证书请求
 */
export interface ApplyCertRequest {
    /**
     * 主域名
     */
    "domain": string;

    /**
     * 备用域名列表
     */
    "sans": string[] | null;

    /**
     * CA 配置 ID
     */
    "ca_id": number;

    /**
     * DNS 提供商 ID（手动 DNS 为 nil）
     */
    "dns_provider_id"?: number | null;

    /**
     * 是否自动续期
     */
    "auto_renew": boolean;

    /**
     * 到期前续期天数
     */
    "renewal_days": number;

    /**
     * 密钥类型：RSA2048/RSA3072/RSA4096/EC256/EC384
     */
    "key_type"?: string;
}

/**
 * ApplyCertResult 证书申请结果
 */
export interface ApplyCertResult {
    /**
     * 是否成功
     */
    "success": boolean;

    /**
     * 证书 PEM 内容
     */
    "cert_content"?: string;

    /**
     * 私钥 PEM 内容
     */
    "key_content"?: string;

    /**
     * 证书颁发者
     */
    "issuer"?: string;

    /**
     * 生效时间
     */
    "not_before"?: string;

    /**
     * 过期时间
     */
    "not_after"?: string;

    /**
     * 错误信息
     */
    "error"?: string;
}

/**
 * CAListItem CA 列表项（前端展示用）
 */
export interface CAListItem {
    /**
     * CA 配置 ID
     */
    "id": number;

    /**
     * CA 名称
     */
    "name": string;

    /**
     * ACME 目录 URL
     */
    "directory_url": string;

    /**
     * 注册邮箱
     */
    "account_email": string;

    /**
     * EAB KID（部分 CA 需要）
     */
    "eab_kid": string;

    /**
     * EAB HMAC Key（部分 CA 需要）
     */
    "eab_hmac": string;

    /**
     * 是否内置 CA
     */
    "is_builtin": boolean;

    /**
     * 是否启用
     */
    "is_active": boolean;

    /**
     * 创建时间
     */
    "created_at": string;

    /**
     * 更新时间
     */
    "updated_at": string;
}

/**
 * CAUpdateRequest 更新 CA 请求
 */
export interface CAUpdateRequest {
    /**
     * CA 名称
     */
    "name"?: string;

    /**
     * ACME 目录 URL
     */
    "directory_url"?: string;

    /**
     * 注册邮箱
     */
    "account_email"?: string;

    /**
     * EAB KID（部分 CA 需要）
     */
    "eab_kid"?: string | null;

    /**
     * EAB HMAC Key（部分 CA 需要）
     */
    "eab_hmac"?: string | null;
}

/**
 * CertificateDetails 证书详细信息（x509 解析结果）
 */
export interface CertificateDetails {
    /**
     * 序列号
     */
    "serial_number": string;

    /**
     * 签名算法
     */
    "signature_algorithm": string;

    /**
     * 公钥算法
     */
    "public_key_algorithm": string;

    /**
     * 公钥位数
     */
    "public_key_size": number;

    /**
     * 密钥用途
     */
    "key_usage": string;

    /**
     * 扩展密钥用途
     */
    "ext_key_usage": string;

    /**
     * 是否 CA 证书
     */
    "is_ca": boolean;

    /**
     * 证书版本
     */
    "version": number;

    /**
     * SHA256 指纹
     */
    "fingerprint_sha256": string;

    /**
     * DNS 名称（SAN）
     */
    "dns_names": string[] | null;

    /**
     * IP 地址（SAN）
     */
    "ip_addresses": string[] | null;

    /**
     * 邮箱地址（SAN）
     */
    "email_addresses": string[] | null;
}

/**
 * CertificateListItem 证书列表项（前端展示用）
 */
export interface CertificateListItem {
    /**
     * 证书 ID
     */
    "id": number;

    /**
     * 主域名
     */
    "domain": string;

    /**
     * 备用域名列表
     */
    "sans": string[] | null;

    /**
     * 证书 PEM 内容
     */
    "cert_content": string;

    /**
     * 私钥 PEM 内容
     */
    "key_content": string;

    /**
     * 证书颁发者
     */
    "issuer": string;

    /**
     * 生效时间
     */
    "not_before": string;

    /**
     * 过期时间
     */
    "not_after": string;

    /**
     * 证书状态
     */
    "status": string;

    /**
     * 是否自动续期
     */
    "auto_renew": boolean;

    /**
     * 到期前续期天数
     */
    "renewal_days": number;

    /**
     * 最后错误信息
     */
    "last_error": string;

    /**
     * 最后续期时间
     */
    "last_renewed_at": string;

    /**
     * 密钥类型
     */
    "key_type": string;

    /**
     * 关联 CA 名称
     */
    "ca_name": string;

    /**
     * 关联 DNS 提供商
     */
    "dns_provider_name": string;

    /**
     * 创建时间
     */
    "created_at": string;

    /**
     * 更新时间
     */
    "updated_at": string;
}

/**
 * CreateCACreateRequest 创建 CA 请求
 */
export interface CreateCACreateRequest {
    /**
     * CA 名称
     */
    "name": string;

    /**
     * ACME 目录 URL
     */
    "directory_url": string;

    /**
     * 注册邮箱
     */
    "account_email": string;

    /**
     * EAB KID（部分 CA 需要）
     */
    "eab_kid": string;

    /**
     * EAB HMAC Key（部分 CA 需要）
     */
    "eab_hmac": string;
}

/**
 * CreateDNSProviderRequest 创建 DNS 提供商请求
 */
export interface CreateDNSProviderRequest {
    /**
     * 提供商名称
     */
    "name": string;

    /**
     * 提供商类型
     */
    "provider_type": string;

    /**
     * 配置参数
     */
    "config": dnsprovider$0.DNSProviderConfig;

    /**
     * 备注
     */
    "comment": string;
}

/**
 * CreateDeployCredentialRequest 创建部署凭证请求
 */
export interface CreateDeployCredentialRequest {
    /**
     * 凭证名称
     */
    "name": string;

    /**
     * 提供商类型
     */
    "provider_type": string;

    /**
     * 配置参数
     */
    "config": deploycredential$0.DeployCredentialConfig;

    /**
     * 备注
     */
    "comment": string;
}

/**
 * CreateDeployTargetRequest 创建部署目标请求
 */
export interface CreateDeployTargetRequest {
    /**
     * 部署目标名称
     */
    "name": string;

    /**
     * 云厂商类型
     */
    "provider_type": string;

    /**
     * 部署服务
     */
    "deploy_service": string;

    /**
     * 部署参数
     */
    "config": deploy$0.DeployTargetConfig;

    /**
     * 凭证来源
     */
    "credential_source": string;

    /**
     * 关联 DNS 凭证 ID（可选）
     */
    "dns_provider_id"?: number | null;

    /**
     * 关联部署凭证 ID（可选）
     */
    "deploy_credential_id"?: number | null;

    /**
     * 是否启用
     */
    "is_active": boolean;

    /**
     * 备注
     */
    "comment": string;
}

/**
 * CurrentCertDTO 云端/面板当前生效证书信息（前端展示用）。
 */
export interface CurrentCertDTO {
    /**
     * CommonName 证书主体名称（CN），即证书绑定的主域名。
     */
    "common_name": string;

    /**
     * SANs 证书主题备用名称列表（Subject Alternative Names），含主域名及所有附加域名。
     */
    "sans": string[] | null;

    /**
     * Issuer 证书颁发者（CA）名称。
     */
    "issuer": string;

    /**
     * NotBefore 证书生效时间（RFC3339 或可读文本）。
     */
    "not_before": string;

    /**
     * NotAfter 证书过期时间（RFC3339 或可读文本）。
     */
    "not_after": string;

    /**
     * SerialNumber 证书序列号（十六进制字符串）。
     */
    "serial_number": string;

    /**
     * Supported 标记该部署目标是否支持查询当前生效证书（不支持时为 false 且 Error 含原因）。
     */
    "supported": boolean;

    /**
     * Error 查询失败时的问题描述（成功时省略）。
     */
    "error"?: string;
}

/**
 * CurrentCertsResultDTO GetCurrentCerts 返回（按资源 key 索引）。
 */
export interface CurrentCertsResultDTO {
    "results": { [_ in string]?: CurrentCertDTO | null } | null;
}

/**
 * DNSProviderListItem DNS 提供商列表项（前端展示用）
 */
export interface DNSProviderListItem {
    /**
     * 提供商 ID
     */
    "id": number;

    /**
     * 提供商名称
     */
    "name": string;

    /**
     * 提供商类型
     */
    "provider_type": string;

    /**
     * 配置参数
     */
    "config": dnsprovider$0.DNSProviderConfig;

    /**
     * 是否启用
     */
    "is_active": boolean;

    /**
     * 备注
     */
    "comment": string;

    /**
     * 创建时间
     */
    "created_at": string;

    /**
     * 更新时间
     */
    "updated_at": string;
}

/**
 * DeployCredentialListItem 部署凭证列表项（前端展示用）
 */
export interface DeployCredentialListItem {
    /**
     * 部署凭证 ID
     */
    "id": number;

    /**
     * 凭证名称
     */
    "name": string;

    /**
     * 提供商类型（见 ent/schema/provider_types.go 的 DeployProviderTypes）
     */
    "provider_type": string;

    /**
     * 配置参数（API 密钥等）
     */
    "config": deploycredential$0.DeployCredentialConfig;

    /**
     * 是否启用
     */
    "is_active": boolean;

    /**
     * 备注
     */
    "comment": string;

    /**
     * 创建时间
     */
    "created_at": string;

    /**
     * 更新时间
     */
    "updated_at": string;
}

/**
 * DeployLogListItem 部署历史列表项（前端展示用）
 */
export interface DeployLogListItem {
    /**
     * 部署历史 ID
     */
    "id": number;

    /**
     * 部署目标名称
     */
    "target_name": string;

    /**
     * 证书 ID
     */
    "cert_id": number;

    /**
     * 证书域名
     */
    "cert_domain": string;

    /**
     * 部署到的加速域名
     */
    "deploy_domain": string;

    /**
     * 云厂商类型
     */
    "provider_type": string;

    /**
     * 部署服务
     */
    "deploy_service": string;

    /**
     * 是否部署成功
     */
    "success": boolean;

    /**
     * 部署结果描述
     */
    "message": string;

    /**
     * 云厂商原始响应（调试用）
     */
    "response": string;

    /**
     * 云端证书 ID
     */
    "cloud_cert_id": string;

    /**
     * 部署时间
     */
    "created_at": string;
}

/**
 * DeployOutcomeDTO 部署结果（前端展示用）
 */
export interface DeployOutcomeDTO {
    /**
     * 部署目标 ID
     */
    "target_id": number;

    /**
     * 部署目标名称
     */
    "target_name": string;

    /**
     * 云端证书 ID
     */
    "cloud_cert_id": string;

    /**
     * 是否部署成功
     */
    "success": boolean;

    /**
     * 部署结果描述
     */
    "message": string;

    /**
     * 云厂商原始响应（调试用）
     */
    "raw_response": string;
}

/**
 * DeployTargetListItem 部署目标列表项（前端展示用）
 */
export interface DeployTargetListItem {
    /**
     * 部署目标 ID
     */
    "id": number;

    /**
     * 部署目标名称
     */
    "name": string;

    /**
     * 云厂商类型（aliyun/tencentcloud/huawei）
     */
    "provider_type": string;

    /**
     * 部署服务（cdn/dcdn/elb/waf 等）
     */
    "deploy_service": string;

    /**
     * 部署参数（region、域名等，按厂商/服务不同）
     */
    "config": deploy$0.DeployTargetConfig;

    /**
     * 区域代码
     */
    "region": string;

    /**
     * 凭证来源（dns_provider/deploy_credential）
     */
    "credential_source": string;

    /**
     * 关联 DNS 凭证 ID（可为空）
     */
    "dns_provider_id": number | null;

    /**
     * 关联 DNS 凭证名称
     */
    "dns_provider_name": string;

    /**
     * 关联部署凭证 ID（可为空）
     */
    "deploy_credential_id": number | null;

    /**
     * 关联部署凭证名称
     */
    "deploy_credential_name": string;

    /**
     * 是否启用
     */
    "is_active": boolean;

    /**
     * 备注
     */
    "comment": string;

    /**
     * 关联证书 ID 列表
     */
    "cert_ids": number[] | null;

    /**
     * 最近一次部署状态（success/failed）
     */
    "last_status": string;

    /**
     * 最近一次部署错误信息
     */
    "last_error": string;

    /**
     * 最近部署时间
     */
    "last_deployed_at": string;

    /**
     * 创建时间
     */
    "created_at": string;

    /**
     * 更新时间
     */
    "updated_at": string;
}

/**
 * FetchCDNDomainsRequest 拉取 CDN 域名请求（内联凭证，用于新建目标时选择）
 */
export interface FetchCDNDomainsRequest {
    /**
     * 云厂商类型
     */
    "provider_type": string;

    /**
     * 部署服务（cdn/dcdn/elb/waf 等）
     */
    "deploy_service": string;

    /**
     * 凭证来源（dns_provider/deploy_credential）
     */
    "credential_source": string;

    /**
     * 关联 DNS 凭证 ID（可选）
     */
    "dns_provider_id"?: number | null;

    /**
     * 关联部署凭证 ID（可选）
     */
    "deploy_credential_id"?: number | null;

    /**
     * 区域代码
     */
    "region": string;

    /**
     * 部署参数（按厂商/服务不同）
     */
    "config": deploy$0.DeployTargetConfig;
}

/**
 * HTMLWindowOptions 通用 HTML 窗口参数
 */
export interface HTMLWindowOptions {
    /**
     * 窗口唯一标识
     */
    "WindowName": string;

    /**
     * 窗口标题
     */
    "Title": string;

    /**
     * HTML 内容
     */
    "Content": string;

    /**
     * 窗口宽度
     */
    "Width": number;

    /**
     * 窗口高度
     */
    "Height": number;

    /**
     * 背景色（CSS 颜色值）
     */
    "BgColor": string;

    /**
     * 文字颜色
     */
    "TextColor": string;

    /**
     * 字体族
     */
    "FontFamily": string;

    /**
     * 字体大小（px）
     */
    "FontSize": number;
}

/**
 * ManualDNSChallenge 手动 DNS 挑战信息
 */
export interface ManualDNSChallenge {
    /**
     * 域名
     */
    "domain": string;

    /**
     * TXT 记录列表
     */
    "records": TXTRecordItem[] | null;
}

/**
 * NotificationItem 通知列表项（前端展示用）
 */
export interface NotificationItem {
    /**
     * 通知 ID
     */
    "id": number;

    /**
     * 通知标题
     */
    "title": string;

    /**
     * 通知内容
     */
    "body": string;

    /**
     * 通知业务分类
     */
    "category": string;

    /**
     * 通知状态（success/error/warning/info）
     */
    "level": string;

    /**
     * 是否已读
     */
    "read": boolean;

    /**
     * 创建时间
     */
    "created_at": string;
}

/**
 * RenewalLogItem 续期日志项（前端展示用）
 */
export interface RenewalLogItem {
    /**
     * 日志 ID
     */
    "id": number;

    /**
     * 关联证书 ID
     */
    "certificate_id": number;

    /**
     * 域名
     */
    "domain": string;

    /**
     * 续期状态
     */
    "status": string;

    /**
     * 错误信息
     */
    "error_message": string;

    /**
     * 新证书 PEM 内容
     */
    "cert_content": string;

    /**
     * 新私钥 PEM 内容
     */
    "key_content": string;

    /**
     * 尝试时间
     */
    "attempt_at": string;

    /**
     * 完成时间
     */
    "completed_at": string;

    /**
     * 创建时间
     */
    "created_at": string;
}

/**
 * TXTRecordItem DNS TXT 记录项
 */
export interface TXTRecordItem {
    /**
     * TXT 记录名称（FQDN）
     */
    "name": string;

    /**
     * TXT 记录值
     */
    "value": string;
}

/**
 * UpdateDNSProviderRequest 更新 DNS 提供商请求
 */
export interface UpdateDNSProviderRequest {
    /**
     * 提供商名称
     */
    "name"?: string;

    /**
     * 提供商类型
     */
    "provider_type"?: string;

    /**
     * 配置参数
     */
    "config": dnsprovider$0.DNSProviderConfig;

    /**
     * 备注
     */
    "comment"?: string;
}

/**
 * UpdateDeployCredentialRequest 更新部署凭证请求
 */
export interface UpdateDeployCredentialRequest {
    /**
     * 凭证名称
     */
    "name": string;

    /**
     * 提供商类型
     */
    "provider_type": string;

    /**
     * 配置参数
     */
    "config": deploycredential$0.DeployCredentialConfig;

    /**
     * 备注
     */
    "comment": string;
}

/**
 * UpdateDeployTargetRequest 更新部署目标请求
 */
export interface UpdateDeployTargetRequest {
    /**
     * 部署目标名称
     */
    "name"?: string;

    /**
     * 云厂商类型
     */
    "provider_type"?: string;

    /**
     * 部署服务
     */
    "deploy_service"?: string;

    /**
     * 部署参数
     */
    "config": deploy$0.DeployTargetConfig;

    /**
     * 凭证来源
     */
    "credential_source"?: string;

    /**
     * 关联 DNS 凭证 ID（可选）
     */
    "dns_provider_id"?: number | null;

    /**
     * 关联部署凭证 ID（可选）
     */
    "deploy_credential_id"?: number | null;

    /**
     * 是否启用
     */
    "is_active"?: boolean | null;

    /**
     * 备注
     */
    "comment"?: string;
}

/**
 * WindowSize 窗口尺寸
 */
export interface WindowSize {
    /**
     * 窗口宽度（像素）
     */
    "width": number;

    /**
     * 窗口高度（像素）
     */
    "height": number;
}
