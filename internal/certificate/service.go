...
func (s *CertificateService) RenewCertificate(ctx context.Context, certID int) (*ApplyCertificateResult, error) {
	...
	logging.Info(i18n.T("log.cert_renew_start", "ID", certID, "Domain", certEntity.Domain))
	...
	// 注册/获取账户：先用 key 查询已有账户；accountDoesNotExist 属正常情况，
	// 随后调用 Register 自动创建账户（创建成功后续期继续，不是失败）
	_, err = client.Registration.ResolveAccountByKey(ctx)
	if err != nil {
		logging.Info(i18n.T("log.register_acme_account_warn", "Error", err))
		...
		logging.Info(i18n.T("log.register_acme_account_ok"))
	}
