...
func initBenchDBMattn(b *testing.B) *sql.DB {
	dsn := sqlite.BuildDSN(":memory:")
	db, err := sql.Open("sqlite3", dsn)
	if err != nil {
		b.Fatalf("open: %v", err)
	}
	if _, err := db.Exec(benchSchemaM); err != nil {
		b.Fatalf("schema: %v", err)
	}
	return db
}

func BenchmarkMattnInsert(b *testing.B) {
	db := initBenchDBMattn(b)
	defer db.Close()
...
func BenchmarkMattnInsertBulk(b *testing.B) {
	db := initBenchDBMattn(b)
	defer db.Close()
...
func BenchmarkMattnSelect(b *testing.B) {
	db := initBenchDBMattn(b)
	defer db.Close()
...
func BenchmarkMattnSelectRows(b *testing.B) {
	db := initBenchDBMattn(b)
	defer db.Close()
...
func BenchmarkMattnUpdate(b *testing.B) {
	db := initBenchDBMattn(b)
	defer db.Close()
...
func BenchmarkMattnDelete(b *testing.B) {
	b.StopTimer()
	db := initBenchDBMattn(b)
	defer db.Close()
...
func BenchmarkMattnQueryLike(b *testing.B) {
	db := initBenchDBMattn(b)
	defer db.Close()
